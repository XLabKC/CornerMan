...
(function() {
   (function(scope) {
      ...
      if ("undefined" === typeof CM_ASSERT_TYPES) CM_ASSERT_TYPES = true;
      if (CM_ASSERT_TYPES) {
         var noop = function() {};
      ...
      ViewModel.createChildObservable = function(viewModel, key, initialValue) {
         ...
         var observable = ko.pureComputed({
            ...
            write: function(child) {
               ...
               if (viewModel.getChildrenForKey(key)[0] !== child) viewModel.replaceChildrenAtKey(key, child ? [ child ] : []);
            },
            owner: viewModel
         });
         observable.key_ = key;
    ...
    */
      ViewModel.createChildrenObservable = function(viewModel, key, initialValue) {
         if (CM_ASSERT_TYPES) cm.assertArgs(arguments, ViewModel, String, cm.optional(cm.arrayOf(ViewModel)));
         ...
         if (initialValue) observable(initialValue);
         return observable;
      };
      ...
      ViewModel.prototype.addListener = function(event, callback) {
         ...
         if (!this.eventsToListeners_[event]) this.eventsToListeners_[event] = [];
      ...
      ViewModel.prototype.addChildAtKey = function(key, viewModel) {
         ...
         if (isNewKey) this.keys_.valueHasMutated(); else this.keysToChildrenObservables_[key].valueHasMutated();
         return true;
      };
    ...
    */
      ViewModel.prototype.removeChildAtKey = function(key, viewModel) {
         ...
         if (wasRemoved) viewModel.parent_(null);
         return wasRemoved;
      };
      /**
    * Attempts to remove the child view model at the given key and returns whether or not it was
    ...
    */
      ...
      var handleChildrenObservableChanged = function(viewModel, key, changes) {
         ...
         for (var i = 0, len = changes.length; i < len; i++) {
            ...
            if (skip) continue;
            if ("added" === change.status) viewModel.addChildAtKey(key, change.value); else if ("deleted" === change.status) viewModel.removeChildAtKey(key, change.value);
         }
      };
   ...
   (function() {
      ...
      var createTemplateAccessorFnForChildren = function(children, ifCondition) {
         if (CM_ASSERT_TYPES) cm.assertArgs(arguments, cm.arrayOf(ViewModel), [ cm.nullable(Function), Boolean ]);
      ...
      var makeTemplateValueAccessor = function(element, valueAccessor, viewModel, asChildren) {
         ...
         if (CM_ASSERT_TYPES) cm.assertOfType(value, [ String, ViewModel, cm.arrayOf(ViewModel), Object ]);
         var child = null;
         var children = null;
         ...
         var captureChildOrChildren = function(value) {
            ...
            child = value; else if (value instanceof Array) {
         ...
         if (value instanceof ViewModel || value instanceof Array) {
            ...
            if ("string" === typeof value) // A key to the child was provided as the value, use the parent view model to fetch the
            // actual child.
   ...
   (function() {
      ...
      Route.prototype.fireCallbacks_ = function(req) {
         ...
         var next = function() {
            callbackIndex++;
            if (callbackIndex < callbackCount) this.callbacks_[callbackIndex](req, next);
         }.bind(this);
   ...
   (function() {
      ...
      ContentViewModel.prototype.getControlsObservableForKeyInternal_ = function(controlKey) {
         if (CM_ASSERT_TYPES) cm.assertArgs(arguments, String);
         if (!this.keysToControlsObservables_[controlKey]) this.keysToControlsObservables_[controlKey] = ko.pureComputed(this.computeControls_.bind(this, controlKey));
